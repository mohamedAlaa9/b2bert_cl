...
def parse_args(argv=None):
    ...
    p.add_argument("--cl_method", type=int, default=1, help="Curriculum learning method (default: 1)")
    p.add_argument("--order", type=str, help="Comma-separated order of stages")
...
def main(argv=None) -> int:
    ...
    if not args.skip_data_prep:
        ...
        try:
            ...
            stage_paths = prepare_curriculum_data(
                ...
                stage_levels=[int(x) for x in args.order.split(",")],
                seed=seed
            )
    ...
    log_info(f"STEP 2: Training Curriculum Sequence (Stages {args.start_stage}-{args.end_stage})")
