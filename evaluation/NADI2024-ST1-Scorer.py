...
def print_usage():
    print(
        """Usage:
    python3 NADI2024-ST1-Scorer.py sample_submission/NADI2024_subtask1_dev2_gold.txt sample_submission/sample_submission_dev_2.txt
        """
    )
...
# Save predictions and ground truths to CSV for later analysis
output_csv = pred_file.split("/")[-1].split(".")[0] + "_predictions_vs_gold.csv"

# Prepare a DataFrame for easier analysis
df = pd.DataFrame()

# Add each country label's true and predicted columns
for i, country in enumerate([COUNTRIES[j] for j in column_indecies_with_labels_in_gold]):
    df[f"{country}_gold"] = gold_matrix[:, i]
    df[f"{country}_pred"] = prediction_matrix[:, i]

# Save to CSV
df.to_csv(output_csv, index=False)
print(f"Saved predictions and gold labels to {output_csv}")