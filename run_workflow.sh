...
EPOCHS=1
BATCH_SIZE=24
DATASET_PATH="/home/mohamed.elzeftawy/Documents/b2bert_cl/aggregated_final/stage_1_and_gpt.csv"
...
exec "$PYTHON_CMD" "$SCRIPT_DIR/main.py"       --exp-num "$EXP_NUM"       --start-stage "$START_STAGE"       --end-stage "$END_STAGE"       --epochs "$EPOCHS"       --batch-size "$BATCH_SIZE"       --dataset_path "$DATASET_PATH"       --order "$ORDER_CSV" --seed "$SEED" --cl_method "$CL_METHOD"   "${OPTS[@]}"
